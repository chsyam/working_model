...
function Navbar() {
  ...
  return (
    <Menu inverted color='blue' stackable size='massive' style={{ borderRadius: 0 }}>
      <Container>
        <Menu.Item header>Book Shop</Menu.Item>
        <Menu.Item as={Link} exact='true' to="/">Home</Menu.Item>
        <Menu.Item as={Link} to="/adminpage" style={adminPageStyle()}>{getUserName() === "AccountManager" ? "AccountManagerPage" : "AdminPage"}</Menu.Item>
